...
public class InterfacePrincipalCuidador extends javax.swing.JFrame {
    ...
    public InterfacePrincipalCuidador(String id, String nome, String tipoContrato, String turno, String especialidades) throws Exception {
        initComponents();
        setarDados(id, nome, tipoContrato, turno, especialidades);
        lerTabelaAgendamentos(id);
    }
    public void lerTabelaAgendamentos(String id) throws Exception{
        DefaultTableModel Agendamentos = (DefaultTableModel) tabelacuidado.getModel();
        Agendamentos.setNumRows(0);
        HashMap<String, UsrPaciente> map = pesquisaAgendamentoCuidador(id);
        if(map!=null){
            Collection<UsrPaciente> pacientes = map.values();
            for(UsrPaciente paciente: pacientes){
